...
function serializeMonster(m) {
    return {
        id: m.id, x: m.x, y: m.y,
        maxHp: m.maxHp, currentHp: m.currentHp,
...
io.on('connection', (socket) => {
    ...
    socket.on('register', async (data) => {
        try {
            ...
            if (insertError) {
                ...
                return socket.emit('authError', `Database Error: ${insertError.message}`);
            }

            socket.emit('registerSuccess', username);
        } catch (err) {
            socket.emit('authError', 'Server encountered an unexpected error.');
        }
    });

    // --- 2. LOGIN (FIXED TO PREVENT HANGS) ---
    socket.on('login', async (data) => {
        try {
            const { username, password } = data;
            if (!username || !password) return socket.emit('authError', 'Invalid username or password.');
            ...
            socket.emit('authError', 'Server encountered an unexpected error during login.');
        }
    });
    ...
    socket.on('createCharacter', async (data) => {
        try {
            ...
            socket.emit('authError', 'Server encountered an unexpected error during creation.');
        }
    });
    ...
    socket.on('enterWorld', (userData) => {
        ...
        onlinePlayers[socket.id] = {
            ...
            mapId: mapId,
            x: userData.pos_x || 960,
            y: userData.pos_y || 1000,
    ...
    // --- MISSING LISTENER FIX: CHAT SYSTEM ---
    socket.on('chatMessage', (data) => {
        const p = onlinePlayers[socket.id];
        if (!p || !data.text) return;
        
        // Broadcast chat to everyone in the same map (Proximity logic is handled strictly by frontend now)
        io.to(p.mapId).emit('chatMessage', { id: p.id, text: data.text });
    });

    // --- MISSING LISTENER FIX: TRADE SYSTEM ---
    socket.on('tradeRequest', ({ targetId }) => {
        const me = onlinePlayers[socket.id];
        if (!me || !targetId) return;

        const targetSid = findSocketIdByPlayerId(targetId);
        if (!targetSid) return socket.emit('partyError', 'Target is not online to trade.');
    ...
    socket.on('tradeInviteResponse', ({ fromId, accept }) => {
        ...
        if (!accept) {
            ...
            io.to(fromSid).emit('partyError', `${me.id} accepted the trade. (Trade window UI logic needed here)`);
        }
    });
    ...
    socket.on('inspectRequest', async ({ targetId }) => {
        ...
        if (target) {
            return socket.emit('inspectData', {
                id: target.id, name: target.name, level: target.level || 1,
                currentHp: target.currentHp ?? null, maxHp: target.maxHp ?? null, equips: target.equips || {}
            });
        }
        ...
        socket.emit('inspectData', {
            id: user.character_name, name: user.character_name, level: user.level || 1,
            currentHp: user.current_hp ?? null, maxHp: null, equips: user.equips || {}
        });
    });
